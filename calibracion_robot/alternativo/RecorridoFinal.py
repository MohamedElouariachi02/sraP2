...
from lib.measurements import analizar_valles, ancho_angular_porteria, calibrar_sensor_color_en_movimiento
...
if mejor_valle is None:
    ...
    exit()
    # elegir el valle con menor distancia
    #mejor_valle = min(valles, key=lambda valle: valle[1])

# acercarse a la porteria
girar(
    grados_robot=mejor_valle[0] if mejor_valle[0] < 180 else mejor_valle[0] - 360,
    velocidad=30,
    block=True
)
recto(
    distancia_m=max(mejor_valle[1] - 35, 0) / 100,
    velocidad=40,
    block=True
)
...
valles = analizar_valles(points, umbral_distancia=50, max_salto=20, grados_totales=180)
...
# celebrar?
girar(180, 50, True)
recto(distancia_porteria/100.0, 20, True)
sound.beep()
leds.all_off()
